---
title: "REPRISE: integrated mechanism, regulatory and prevalence scoring for drug repurposing"
author:
...
Computational drug-repurposing screens typically end at a mechanism score, leaving the regulatory, market and competitive context that determine actionability untested. We present REPRISE (Repurposing Engine for Pathway-Resolved Indication Scoring and Evidence), an end-to-end pipeline that scores 3,996 approved drugs against 28,198 diseases (176,272 hypotheses) by combining noisy-OR mechanism support with directionality, tissue expression, pathway co-membership, FDA Orange Book IP runway, Orphanet rare-disease prevalence and a literature-and-patent prior across PubMed, Europe PMC, ClinicalTrials.gov and Lens.org computed across every ranked hypothesis. Validated against 54 curated repurposing wins, REPRISE recovers 48 (89%, 95% Clopper–Pearson CI 77–96%) at mechanism-support 0.3; all six misses trace to Open Targets coverage gaps, not pipeline errors. The screen rediscovered the BRAF + MEK doublet *de novo* in cardiofaciocutaneous syndrome and surfaced metformin's mitochondrial complex-I mechanism in polycystic ovary syndrome. End-to-end runtime is approximately 12 minutes; a public dashboard (https://reprise.streamlit.app) exposes the full ranked output.
...
The literature pass ran across **every one of the 176,263 ranked hypotheses** (not a top-N subset), querying PubMed, Europe PMC, ClinicalTrials.gov, and the Lens.org patent backend in parallel and deduplicating queries at the (target, disease) level (Methods). Across the full screen, 83,042 hypotheses (47.1%) carried PubMed evidence, 90,123 (51.1%) Europe PMC evidence, 61,721 (35.0%) at least one registered clinical trial, and 3,008 (1.7%) at least one Lens.org patent filing matched to the substance–disease pair; the four signals were log-saturated and combined into `investigation_prior ∈ [0, 1]` which damps opportunity by `(1 − 0.5 × investigation_prior)`. The Lens patent coverage was capped by the free-tier monthly query quota rather than the pipeline (the cheap sources ran to completion); a commercial Lens contract would lift this cap with no other change required. The patent prior carried real signal: the most-patented hypotheses were satralizumab → rheumatoid arthritis (125 filings, `investigation_prior` 0.99, demoted to rank 1,547), tralokinumab and lebrikizumab → psoriasis (20 filings each, prior 0.88), and the KRAS-G12C inhibitors adagrasib and sotorasib → cardiofaciocutaneous syndrome (18 filings each, prior 0.48) — separating mechanisms with material concurrent industry activity from genuinely under-investigated ones. The combined literature-and-patent prior carries through to the per-hit PDF briefs, where it is rendered as an explicit "investigation density" line so a reviewer can see at a glance whether the screen's hit sits in an active or quiet competitive space.
...
**Literature pass.** Per unique (target, disease) pair across all ranked hypotheses, query PubMed/Europe PMC/ClinicalTrials.gov/Lens with synonym-expanded queries. Deduplication on (target, disease) keeps issued query count bounded by unique pair count (~30,000 across the full 176,263-row screen). Counts log-saturated and combined to `investigation_prior ∈ [0, 1]`. Opportunity damped by `(1 − 0.5 × investigation_prior)`. Results cached locally (90-day TTL) so re-runs are near-instant; Lens.org coverage is rate-limited by free-tier monthly quota in the reported run.
...
**Supplementary Fig. 2 — Public Streamlit dashboard** (live at **https://reprise.streamlit.app**; source under `dashboard/` in https://github.ugent.be/wvcrieki/repurposed). Interactive companion to the manuscript that exposes the full 176,272-row ranked output through three tabs (Browse / History / FAQ), with sidebar filters, an opportunity-vs-mech scatter, per-hit detail panels rendering 2D structures from ChEMBL, live PubMed search links, and the bundled PDF briefs. The History tab carries the timeline of the 54-case validation set and the disease-disease network. The dashboard bundles a frozen parquet snapshot of the screen's full output.
